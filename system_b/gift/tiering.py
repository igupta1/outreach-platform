...
from system_b.gift.engine import SIGNAL_RANK, build_gift
...
def _supply_key(gift: Gift) -> tuple[int, int]:
    """Rank candidate gifts by lead supply: more leads first, then a stronger
    best-lead signal. All contenders are already all-niche."""
    return (gift.gift_size, -SIGNAL_RANK.get(gift.best_lead.signal_type, 9))
...
def resolve_gift(
    ...
    *, fit: FitFn | None = None,
    ...
    for mp, phrase in _candidate_pairs(research):
        ...
        g = build_gift(trial, scraper, niche_only=True)          # GATE B (taxonomy)
        ...
        if best is None or _supply_key(g) > _supply_key(best[0]):
            best = (g, mp, phrase)
    ...
    return prospect, build_gift(prospect, scraper)
